...
function signingSecret() { return String(process.env.ORDER_REFERENCE_SECRET || ""); }
function expectedSignature(pathname, exp, secret) { return createHmac("sha256", secret).update(`${pathname}.${exp}`).digest("hex"); }
...
export default async function handler(request) {
  ...
  if (!secret || !/^custom-orders\/reference-[A-Za-z0-9._-]+$/.test(pathname) || !/^\d{13}$/.test(exp) || !/^[a-f0-9]{64}$/.test(sig)) return new Response("Invalid private reference link.", { status: 403 });
