...
const uploadBuckets = globalThis.__VELVET_NETLIFY_ART_GIFTS_UPLOAD_BUCKETS || new Map();
globalThis.__VELVET_NETLIFY_ART_GIFTS_UPLOAD_BUCKETS = uploadBuckets;
...
function json(value, status = 200, extraHeaders = {}) { return Response.json(value, { status, headers: extraHeaders }); }
...
export default async function handler(request, context) {
  ...
  try {
    ...
    if (!(photo instanceof File) || !ALLOWED_TYPES.has(photo.type) || photo.size <= 0 || photo.size > MAX_FILE_SIZE) return json({ error: "Please upload a JPG, PNG or WEBP photo under 4 MB." }, 400);
    ...
    await store.set(pathname, await photo.arrayBuffer(), { metadata: { contentType: photo.type, originalName: String(photo.name || "reference photo").slice(0, 200), createdAt: Date.now() }, onlyIfNew: true });
    return json({ pathname });
  } catch (error) {
    console.error("Netlify checkout photo upload error:", error);
