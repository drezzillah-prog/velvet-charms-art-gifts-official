...
function createResponseShim() {
  ...
  const res = {
    ...
    json(value) { headers.set("Content-Type", "application/json; charset=utf-8"); payload = JSON.stringify(value); ended = true; res.headersSent = true; return res; },
    send(value = "") { payload = typeof value === "string" || value instanceof Uint8Array ? value : JSON.stringify(value); ended = true; res.headersSent = true; return res; },
    end(value = "") { payload = value; ended = true; res.headersSent = true; return res; },
    destroy() { ended = true; }
  };
  return { res, response() { if (!ended && statusCode === 204) return new Response(null, { status: statusCode, headers }); return new Response(payload ?? "", { status: statusCode, headers }); } };
}
...
export const config = { path: ["/api/create-order", "/api/capture-order", "/api/contact", "/api/currency", "/api/ping"] };
