...
(() => {
  ...
  function polished(value,l){
    const raw=String(value||''),clean=raw.trim();
    if(!clean||l==='en')return raw;
  ...
  function apply(root=document.body){
    if(!root)return;const l=lang();if(l==='en')return;
    const scan=node=>{
      const w=document.createTreeWalker(node,NodeFilter.SHOW_TEXT);
      while(w.nextNode()){
        const n=w.currentNode;if(!n.nodeValue.trim())continue;
        const next=polished(n.nodeValue,l);
  ...
  let queued=false;
  function schedule(){if(queued)return;queued=true;queueMicrotask(()=>{queued=false;apply(document.body);});}
