...
async function run({tamper=false,alreadyCompleted=false,captureFailure=false,invalidMarket=false,badCompletedAmount=false}={}){
  ...
  global.fetch=async(url)=>{
    ...
    if(u.endsWith('/v2/checkout/orders/ORDER1')) {
      ...
      return {ok:true,status:200,json:async()=>({
        ...
        purchase_units:[{
          ...
          items:[{sku:'landscape_small',quantity:'1',unit_amount:{currency_code:'EUR',value}}],
          amount:{currency_code:'EUR',value},
          ...(alreadyCompleted?{payments:{captures:[{id:'CAP-EXISTING',amount:{currency_code:'EUR',value:badCompletedAmount?'44.00':'45.00'}}]}}:{})
        }]
      })};
    }
    if(u.endsWith('/v2/checkout/orders/ORDER1/capture')){
      ...
      return {ok:true,status:201,json:async()=>({id:'ORDER1',status:'COMPLETED',purchase_units:[{payments:{captures:[{id:'CAP1',amount:{currency_code:'EUR',value:'45.00'}}]}}]})};
    }
...
console.log('PASS: Art & Gifts validates approved pricing before capture, rejects tampering, handles PayPal failures, and safely recovers already-completed orders without charging twice.');
