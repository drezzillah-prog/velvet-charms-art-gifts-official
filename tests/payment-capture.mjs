...
function fingerprint(){return createHash('sha256').update(JSON.stringify({items:[{id:'landscape_small',quantity:1,options:{},attachments:[]}],requestedDate:''})).digest('hex').slice(0,40);}
...
async function run({tamper=false,alreadyCompleted=false,captureFailure=false,invalidMarket=false,badCompletedAmount=false,badFingerprint=false}={}){
  ...
  global.fetch=async(url)=>{
    ...
    if(u.endsWith('/v2/checkout/orders/ORDER1')) {
      const value=tamper?'0.01':'45.00';
      const fp=badFingerprint?'0'.repeat(40):fingerprint();
      return {ok:true,status:200,json:async()=>({id:'ORDER1',status:alreadyCompleted?'COMPLETED':'APPROVED',purchase_units:[{custom_id:`${invalidMarket?'XX':'INTL'}:${fp}`,items:[{sku:'landscape_small',quantity:'1',unit_amount:{currency_code:'EUR',value}}],amount:{currency_code:'EUR',value},...(alreadyCompleted?{payments:{captures:[{id:'CAP-EXISTING',amount:{currency_code:'EUR',value:badCompletedAmount?'44.00':'45.00'}}]}}:{})}]})};
    }
    if(u.endsWith('/v2/checkout/orders/ORDER1/capture')){captureCalls++;if(captureFailure)return{ok:false,status:422,json:async()=>({name:'UNPROCESSABLE_ENTITY'})};return{ok:true,status:201,json:async()=>({id:'ORDER1',status:'COMPLETED',purchase_units:[{payments:{captures:[{id:'CAP1',amount:{currency_code:'EUR',value:'45.00'}}]}}]})};}
    throw new Error(`Unexpected fetch ${u}`);
  };
  const response=res();
  await captureOrder({method:'POST',headers:{host:'preview.test'},body:{orderID:'ORDER1',cart:{items:[{id:'landscape_small',qty:1,options:{}}]}}},response);
...
console.log('PASS: Art & Gifts binds approval to exact cart details, rejects price/detail tampering, handles PayPal failures, and recovers completed orders without charging twice.');
