...
async function run({tamper=false}={}){
  ...
  global.fetch=async(url)=>{
    ...
    if(u.endsWith('/v2/checkout/orders/ORDER1')) return {ok:true,status:200,json:async()=>({purchase_units:[{custom_id:'INTL',items:[{sku:'landscape_small',quantity:'1',unit_amount:{currency_code:'EUR',value:tamper?'0.01':'45.00'}}],amount:{currency_code:'EUR',value:tamper?'0.01':'45.00'}}]})};
    if(u.endsWith('/v2/checkout/orders/ORDER1/capture')){captureCalls++;return {ok:true,status:201,json:async()=>({id:'ORDER1',status:'COMPLETED',purchase_units:[{payments:{captures:[{id:'CAP1',amount:{currency_code:'EUR',value:'45.00'}}]}}]})};}
    throw new Error(`Unexpected fetch ${u}`);
  };
  const response=res();
  await captureOrder({method:'POST',body:{orderID:'ORDER1',cart:{items:[{id:'landscape_small',qty:1,options:{}}]}}},response);
...
console.log('PASS: Art & Gifts PayPal capture validates SKU, quantity, currency, unit price and total before collecting funds.');
