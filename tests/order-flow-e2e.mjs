...
const pricingRo = JSON.parse(readFileSync(join(process.cwd(), 'pricing-ro.json'), 'utf8'));
...
async function captureCheckout(cart, createdBody, {
  ...
  if (completedAlready) {
    details.purchase_units[0].payments = { captures: [{ id: 'CAPTURE-OLD', amount: { currency_code: 'EUR', value: total } }] };
  }
    ...
    if (href.endsWith(`/v2/checkout/orders/${orderID}/capture`)) {
      ...
      return {
        ...
        json: async () => ({
          ...
          purchase_units: [{ payments: { captures: [{ id: 'CAPTURE-1', amount: { currency_code: 'EUR', value } }] } }]
        })
      };
    }
...
const simpleProduct = products[0];
assert.ok(simpleProduct?.id, 'expected Art & Gifts test product');
...
const expectedRoEur = Number((Number(pricingRo[simpleProduct.id]) / 5).toFixed(2));
assert.equal(roCreate.createdBody.purchase_units[0].items[0].unit_amount.value, expectedRoEur.toFixed(2));
...
const optionProduct = products.find(product => product.options && Object.keys(product.options).some(key => Array.isArray(product.options[key]) && product.options[key].length));
assert.ok(optionProduct, 'expected at least one customizable Art & Gifts product');
const optionEntries = Object.entries(optionProduct.options || {}).filter(([, values]) => Array.isArray(values) && values.length);
const chosenOptions = Object.fromEntries(optionEntries.slice(0, 2).map(([key, values]) => [key, values[0]]));
chosenOptions.special_instructions = 'Keep the finish elegant and faithful to the reference.';
const otherProducts = products.filter(product => product.id !== optionProduct.id).slice(0, 2);
assert.equal(otherProducts.length, 2, 'expected enough products for a multi-item cart');
const customCart = {
  ...
  items: [
    {
      ...
      attachments: [{ pathname: 'custom-orders/reference-test-one.jpg', name: 'reference.jpg' }]
    },
    { id: otherProducts[0].id, qty: 1, options: {} },
    { id: otherProducts[1].id, qty: 1, options: {} }
  ]
};
...
assert.match(customCreate.createdBody.purchase_units[0].items[0].description, /private reference photo/);
...
console.log('PASS: Art & Gifts create→approve→capture flow, RO/INTL pricing, multi-item customization, photos, preferred date, tamper protection, failure handling and idempotent recovery are intact.');
