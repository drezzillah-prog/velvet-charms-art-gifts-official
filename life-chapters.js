...
for(const [world,groups] of Object.entries(worlds)){const root=document.querySelector('[data-world="'+world+'"]');if(!root)continue;root.innerHTML=groups.map(g=>'<article class="chapter-group"><h4>'+esc(g.title)+'</h4><p>'+esc(g.note)+'</p><ul class="idea-list">'+g.items.map(i=>'<li>'+esc(i)+'</li>').join('')+'</ul></article>').join('');}
})();