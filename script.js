(function () {
  ...
  const HIDDEN_BUNDLE_IDS = new Set(["bundle_relax_restore", "bundle_cozy_winter", "bundle_home_harmony"]);
  ...
  function buildGallery(product) {
    ...
    if (product.images && product.images.length) {
      ...
      if (product.images.length > 1) {
        ...
        product.images.slice(1).forEach(img => { gallery += `<img src="${escapeHtml(img)}" alt="${escapeHtml(product.name)} example" loading="lazy" decoding="async">`; });
        gallery += "</div>";
      }
    }
    return gallery;
  ...
  function categoryHasVisibleProducts(category) {
    if (visibleProducts(category.products).length) return true;
    return (category.subcategories || []).some(sub => visibleProducts(sub.products).length);
  }
  ...
  function renderCatalogue(data) {
    ...
    (data.categories || []).filter(categoryHasVisibleProducts).forEach(category => {
      ...
      if (Array.isArray(category.subcategories)) category.subcategories.forEach(sub => {
        const products = visibleProducts(sub.products);
        if (!products.length) return;
        html += `<h3 class="catalogue-sub">${escapeHtml(sub.name)}</h3><div class="catalogue-grid">`;
        products.forEach(product => { html += productCard(product, category.name, sub.name); });
        html += "</div>";
      });
      const products = visibleProducts(category.products);
      if (products.length) {
        html += '<div class="catalogue-grid">';
        products.forEach(product => { html += productCard(product, category.name); });
        html += "</div>";
      }
      html += "</section>";
    });
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try { renderCatalogue(await loadCatalogue()); }
    catch (err) {
      console.error(err);
      const root = document.getElementById("catalogue-root");
      if (root) { root.classList.remove("loading"); root.innerHTML = '<p class="catalogue-error">The catalogue could not be loaded. Please refresh the page.</p>'; }
    }
  });

  document.addEventListener("click", function (event) {
    if (event.target.matches(".thumbs img")) {
      const clicked = event.target, card = clicked.closest(".product-card");
      if (!card) return;
  ...
  function openLightbox(src, alt) {
    ...
    if (!lightbox) {
      lightbox = document.createElement("div"); lightbox.className = "lightbox hidden";
      lightbox.innerHTML = '<img src="" alt=""><button class="lightbox-close" type="button" aria-label="Close image">×</button>';
      document.body.appendChild(lightbox);
      lightbox.addEventListener("click", event => { if (event.target === lightbox || event.target.matches(".lightbox-close")) lightbox.classList.add("hidden"); });
    }
